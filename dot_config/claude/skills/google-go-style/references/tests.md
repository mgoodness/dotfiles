...
- **Test helper** — performs setup or cleanup. Failure means _the test environment_ is broken (no free port, can't write a file). Calls `t.Helper()` so failure points at the test's call site, not the helper's.
