...
| `run_after_17-herdr-setup.sh`                         | herdr: binary, launchd service, agent skill, Pi integration, worktrunk plugin |
...
> Ghostty installs via Homebrew (`cask "ghostty"`); herdr installs via its own installer (`run_after_17-herdr-setup.sh`, not Homebrew — the curl-installed binary is what supports `herdr update --handoff`).
